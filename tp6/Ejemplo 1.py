...
# Los datos pueden hasta venir en distinto orden cuando se ingresa
    try:
        ...
        while nombre!="":
            ...
            nombre=nombre[:99]
            #Armo el registro para guardar en el archivo
            registro=f'{len(str(legajo)):02d}{legajo}' #campo
            #Al hacer f-string se puede tomar e ingresar directamente
            registro+=f'{len(nombre):02d}{nombre}'
            registro+=f'{len(categoria):02d}{categoria}'
            registro+=f'{len(str(sueldo)):02d}{sueldo}'
            #El formato ya transforma numero en cadena
            #Se puede usar el formato para decirle a cada campo como quiero rellenar
            arch.write(registro+'\n')
